...
class CombinedConstraint(torch.nn.Module):
    ...
    """
    def __init__(self, constraint_params, device='cuda'):
    ...
    def _should_apply_at_iter(self, constraint_name, niter):
        ...
        if constraint_name not in self.constraint_params:
            return False
    ...
    @property
    def loss_names(self):
        """Names of differentiable constraint losses reported with other losses."""
        names = []
        for name in ("tv_weight_z", "surface_zero_weight"):
            if name not in self.constraint_params:
                continue
            params = self.constraint_params[name]
            if params.get("start_iter") is not None and params.get("weight", 0.0) != 0:
                names.append(name)
        return names
    ...
    def get_loss_tv_weight_z(self, model, niter):
        """Differentiable TV penalty between neighboring object phase slices."""
        name = "tv_weight_z"
        if not self._should_apply_at_iter(name, niter):
            return self._zero_loss()

        weight = self.constraint_params[name].get("weight", 0.0)
        if weight == 0:
            return self._zero_loss()

        objp = model.opt_objp
        if objp.ndim < 4 or objp.shape[1] < 2:
            return self._zero_loss()

        tv_per_omode = objp.diff(dim=1).abs().mean(dim=(1, 2, 3))
        return weight * (tv_per_omode * model.omode_occu).sum()

    def get_loss_surface_zero_weight(self, model, niter):
        """Differentiable penalty that suppresses phase on entrance/exit slices."""
        name = "surface_zero_weight"
        if not self._should_apply_at_iter(name, niter):
            return self._zero_loss()
        ...
        objp = model.opt_objp
        if objp.ndim < 4 or objp.shape[1] < 3:
            return self._zero_loss()

        surface_per_omode = objp[:, 0].abs().mean(dim=(1, 2)) + objp[:, -1].abs().mean(dim=(1, 2))
        return weight * (surface_per_omode * model.omode_occu).sum()

    def get_constraint_losses(self, model, niter):
        """Return differentiable constraint loss tensors in loss_names order."""
        losses = []
        if "tv_weight_z" in self.loss_names:
            losses.append(self.get_loss_tv_weight_z(model, niter))
        if "surface_zero_weight" in self.loss_names:
            losses.append(self.get_loss_surface_zero_weight(model, niter))
        return losses

    def get_constraint_loss(self, model, niter):
        """Return total differentiable constraint loss and its components."""
        losses = self.get_constraint_losses(model, niter)
        total_loss = sum(losses) if losses else self._zero_loss()
    ...
    def apply_identical_slices(self, model, niter):
        '''Apply identical-slices constraint by replacing each z slice with the z-mean slice.'''
        # This keeps obj_Nlayer > 1 and the multislice propagator active, while forcing
        # the object transmission function to be identical at every depth slice.

        if self._should_apply_at_iter('identical_slices', niter):
            obj_type = self.constraint_params['identical_slices'].get('obj_type', 'both')
            relax = self.constraint_params['identical_slices'].get('relax', 0.0)

            if model.opt_objp.shape[1] < 2:
                logger.debug("Skip identical_slices constraint because object has only one z slice")
                return

            if obj_type in ['amplitude', 'both']:
                obja_mean = model.opt_obja.mean(dim=1, keepdim=True)
                model.opt_obja.copy_(relax * model.opt_obja + (1 - relax) * obja_mean)

            if obj_type in ['phase', 'both']:
                objp_mean = model.opt_objp.mean(dim=1, keepdim=True)
                model.opt_objp.copy_(relax * model.opt_objp + (1 - relax) * objp_mean)

            relax_str = f'relaxed ({relax}*obj + ({1-relax}*zmean))' if relax != 0 else 'hard'
            logger.debug(f"Apply {relax_str} identical_slices constraint on {obj_type} at iter {niter}")
    ...
    def forward(self, model, niter):
        ...
        with torch.no_grad():
            ...
            self.apply_identical_slices(model, niter)
