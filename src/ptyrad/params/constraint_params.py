...
class TvWeightZ(BaseModel):
    model_config = {"extra": "forbid"}

    start_iter: Optional[int] = Field(default=None, ge=1, description="Start iteration of applying z-direction TV loss")
    step: Optional[int] = Field(default=1, ge=1, description="Interval of iterations of applying z-direction TV loss")
    end_iter: Optional[int] = Field(default=None, ge=1, description="End iteration of applying z-direction TV loss")
    weight: float = Field(default=0.0, ge=0.0, description="Weight for z-direction TV loss on object phase")


class SurfaceZeroWeight(BaseModel):
    model_config = {"extra": "forbid"}

    start_iter: Optional[int] = Field(default=None, ge=1, description="Start iteration of applying surface-zero loss")
    step: Optional[int] = Field(default=1, ge=1, description="Interval of iterations of applying surface-zero loss")
    end_iter: Optional[int] = Field(default=None, ge=1, description="End iteration of applying surface-zero loss")
    weight: float = Field(default=0.0, ge=0.0, description="Weight for suppressing object phase on entrance and exit slices")


class IdenticalSlices(BaseModel):
    model_config = {"extra": "forbid"}

    start_iter: Optional[int] = Field(default=None, ge=1, description="Start iteration of applying identical-slices constraint")
    step: Optional[int] = Field(default=1, ge=1, description="Interval of iterations of applying identical-slices constraint")
    end_iter: Optional[int] = Field(default=None, ge=1, description="End iteration of applying identical-slices constraint")
    obj_type: Literal["amplitude", "phase", "both"] = Field(default="both", description="Object type to constrain")
    relax: float = Field(default=0.0, ge=0.0, le=1.0, description="Relaxation parameter. 0 is hard z-mean replacement; 1 disables the update")
...
class ConstraintParams(BaseModel):
    ...
    tv_weight_z: TvWeightZ = Field(
        default_factory=TvWeightZ, description="Z-direction TV loss for object phase"
    )
    """
    Add a differentiable total-variation loss between neighboring object phase slices.
    Unlike obj_zblur, this is a soft loss term added to the ptychography objective instead of an in-place blur.
    Set start_iter to a positive integer and weight to a nonzero value to enable it.
    """

    surface_zero_weight: SurfaceZeroWeight = Field(
        default_factory=SurfaceZeroWeight, description="Surface-zero loss for object phase"
    )
    """
    Add a differentiable loss that suppresses object phase on the first and last z slices.
    This encourages the object potential to stay near zero at the slab entrance and exit surfaces.
    Set start_iter to a positive integer and weight to a nonzero value to enable it.
    """

    identical_slices: IdenticalSlices = Field(
        default_factory=IdenticalSlices, description="Force object slices to be identical"
    )
    """
    Apply a hard z-mean constraint so every object slice is identical while keeping obj_Nlayer > 1.
    This approximates quantEM's identical_slices=True behavior: multislice propagation remains active,
    but each z slice uses the same averaged object. Set start_iter to a positive integer to enable it.
    """
